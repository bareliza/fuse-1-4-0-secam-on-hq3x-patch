...
#define GREENMASK9   ( greenMask   )
  if( settings_current.pal_tv2x ) {
    q0 = (scaler_data_type *)dstPtr;
    for( i = 0; i < 4*height; i += 4 ) {
      j = 4*width;
      while(j >= 0) {
	q0[j] =
	  ( ( ( ( q0[j    ] & REDBLUEMASK9 )*7 ) >> 3 ) & REDBLUEMASK9 ) |
	  ( ( ( ( q0[j    ] & GREENMASK9   )*7 ) >> 3 ) & GREENMASK9   );
	--j;
      }
      q0 += 4*nextlineDst;
    }
...
//#define GREENMASK91 (0x3f << 5)
#define GREENMASK91 (0x1f << 6)
    q0 = (scaler_data_type *)dstPtr;
    for( i = 0; i < 4*height; i++ ) {
      j = 4*width - SECAM_SYNC2 - 1;
      while(j > 0) {
	if ( q0[j - 1] != q0[ j ] ) {
	...
	( ( ( ( q0[j + k] & GREENMASK91   )*7 ) >> 3 ) & GREENMASK91  );
	    } else {
	      q0[j + k] =
	( ( ( ( q0[j + k] & REDBLUEMASK9 )*3 ) >> 2 ) & REDBLUEMASK9 ) |
	( ( ( ( q0[j + k] & GREENMASK91  )*3 ) >> 2 ) & GREENMASK91  );
	    }
	} 
	--j;
      }
...
#define SECAM_SYNC (settings_current.pal_tv2x - 4)
    q0 = (scaler_data_type *)dstPtr;
    for( i = 0; i < 4*height; i++ ) {
      for(j = SECAM_SYNC; j < 4*width - 1; ++j) {
	if ( q0[j + 1] != q0[j] ) {
