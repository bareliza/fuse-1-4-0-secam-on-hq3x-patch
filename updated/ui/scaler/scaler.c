...
static void expand_pal( int *x, int *y, int *w, int *h,
			int image_width, int image_height );
static void expand_pal_hq( int *x, int *y, int *w, int *h,
			int image_width, int image_height );
...
static const struct scaler_info available_scalers[] = {
  ...
  { "HQ 2x",           "hq2x",      SCALER_FLAGS_EXPAND,       2.0,
    scaler_HQ2x_16,       scaler_HQ2x_32,       expand_pal          },
  { "HQ 3x",           "hq3x",      SCALER_FLAGS_EXPAND,       3.0,
    scaler_HQ3x_16,       scaler_HQ3x_32,       expand_pal_hq       },
};
...
#define SECAM_SYNC (9)  
static void
expand_pal_hq( int *x, int *y, int *w, int *h, int
	       image_width, int image_height )
{
  (*x)-=(3*SECAM_SYNC*2); (*w)+=(2 * 3*SECAM_SYNC*2);
  clip( x, y, w, h, image_width, image_height );
}
